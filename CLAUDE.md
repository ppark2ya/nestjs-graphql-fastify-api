...
Nx monorepo 기반의 멀티 서버 + 프론트엔드 프로젝트. 여섯 개의 애플리케이션으로 구성된다:
- **Nginx** (port 80→4000): UI 정적 파일 서빙 + 리버스 프록시. `/admin` 경로 IP whitelist, `/graphql`·`/api` 요청을 Gateway로 프록시한다. Apache → Nginx → Gateway 구조.
- **Gateway** (port 4000, 내부 전용): GraphQL API 게이트웨이. 외부 REST API를 GraphQL 인터페이스로 통합하여 단일 엔드포인트로 제공한다. GraphQL Subscription (Redis PubSub)을 지원한다.
...
- **UI** (port 5173 dev): React SPA 대시보드. 로그인/2FA 인증, 실시간 Docker 로그 스트리밍, 로그 검색 기능을 제공한다. 프로덕션에서는 Nginx가 빌드된 정적 파일을 서빙한다.
...
├── docker-stack.test.yml                # Swarm 통합 테스트 Stack (7 서비스)
├── docker/
│   └── nginx/
...
- 별도 Docker Swarm 서비스로 운영. UI 빌드 결과물을 Nginx 이미지에 포함 (multi-stage build).
- **요청 흐름**: `Apache → Nginx (:80→4000) → Gateway (:4000, 내부)`
- `/admin` 경로: `geo` 모듈 기반 IP whitelist 적용 (`docker/nginx/whitelist.conf`)
- `/graphql`: Gateway로 프록시 (HTTP + WebSocket upgrade 지원)
- `/api`: Gateway로 프록시
- `/*`: React SPA 정적 파일 + `try_files` fallback
- Gateway는 외부 포트를 노출하지 않으며, Nginx를 통해서만 접근 가능
- **CORS**: SPA와 API가 같은 Nginx origin에서 서빙되므로 same-origin 요청 (CORS 이슈 없음)
...
services:
  ...
  nginx:
    image: ${DOCKER_REPO_NGINX}:${TAG}
    ports: ["4000:80"]
    depends_on: [gateway]

  gateway:
    image: ${DOCKER_REPO_GATEWAY}:${TAG}
    # 외부 포트 노출 없음 (nginx를 통해서만 접근)
    environment:
...
| `DOCKER_REPO_NGINX` | Nginx 이미지 저장소 | - |
| `TAG` | 이미지 태그 | `latest` |
...
단일 Stack (`test-app`)으로 7개 서비스를 overlay network에 배치:

| 서비스 | 이미지 | 레플리카 | 포트 | 역할 |
|--------|--------|----------|------|------|
| redis | redis:7-alpine | 1 | 내부 | PubSub, 캐시 |
| mysql | mysql:8 | 1 | 내부 | auth DB |
| nginx | nginx-ui:test | 1 | 4000 (ingress) | UI 서빙 + 리버스 프록시 |
| gateway | gateway:test | 2 | 내부 | GraphQL API |
